...
const { VALID_SCOPES, enabledAgents, getAgent, resolveAgentIds, resolveScope } = require('./agent-registry');
const {
...
async function promptForScopeLegacy({ input = process.stdin, output = process.stdout } = {}) {
	output.write('\nInstall scope:\n');
...
async function promptForAgentsLegacy({ input = process.stdin, output = process.stdout } = {}) {
	const agents = enabledAgents();
	output.write('Select agents:\n');
...
async function run(
	...
	if (parsed.command === 'uninstall') {
		const agentIds = resolveInstallAgentIds(parsed, { env, interactive: input.isTTY });
		const scopes = parsed.scope ? [resolveInstallScope(parsed, { env })] : VALID_SCOPES.slice();
		...
		for (const result of results) {
			if (result.removed) {
				...
				output.write(
					`Skipped ${result.displayName} skill (${result.scope}) — not installed at ${result.targetRoot}\n`
				);
			}
		}
		return 0;
...
module.exports = {
	...
	printStatus,
	promptForAgents,
	promptForAgentsLegacy,
	promptForScope,
	promptForScopeLegacy,
